...
try {
    uv run uvicorn home.asgi:application --host $ServerIP --port $Port
} catch [System.Management.Automation.PipelineStoppedException] {
    # Interruzione normale con Ctrl+C - non è un errore
    Write-Host "Server fermato dall'utente." -ForegroundColor Yellow
} catch {
    Write-Host "Errore avvio server: $($_.Exception.Message)" -ForegroundColor Red
}
