...
class GroupsPermissionsTest(TestCase):
    ...
    def test_full_access_group_creation(self):
        ...
        self.assertEqual(full_access_group.permissions.count(), all_permissions.count())
...
class AdminIntegrationTest(TestCase):
    ...
    def test_admin_user_creation(self):
        ...
        self.assertTrue(CustomUser.objects.filter(email="new.user@aslcn1.it").exists())
...
class PerformanceTest(TestCase):
    ...
    def test_query_optimization(self):
        ...
        list(CustomUser.objects.select_related().all()[:5])

        # Verifica che non ci siano troppe query
        query_count = len(connection.queries)
        self.assertLess(query_count, 10, f"Troppe query: {query_count}")
